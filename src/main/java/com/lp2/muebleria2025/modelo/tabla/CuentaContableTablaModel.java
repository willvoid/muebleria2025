/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package com.lp2.muebleria2025.modelo.tabla;

import com.lp2.muebleria2025.modelo.CuentaContable;
import java.util.List;
import javax.swing.table.AbstractTableModel;

/**
 *
 * @author cmendieta
 */
public class CuentaContableTablaModel extends AbstractTableModel{
    List<CuentaContable> lista;
    private String[] columnas ={"ID","NOMBRE","TIPO"};
    
    public void setLista(List<CuentaContable> lista){
        // Inicializamos las lista de cuentas
        this.lista = lista;
    }

    @Override
    public int getRowCount() {
        // El tamaño de la lista
        return lista.size();
    }

    @Override
    public int getColumnCount() {
        // El numero de columnass
       return columnas.length;
    }

    @Override
    public Object getValueAt(int fila, int columna) {
        CuentaContable cuenta = lista.get(fila);
        switch (columna) {
            case 0:
                return cuenta.getId();
            case 1:
                return cuenta.getNombre();
            case 2:
                return cuenta.getTipo();
            default:
                return null;
        }
    }

    @Override
    public String getColumnName(int column) {
        return columnas[column]; 
    }
    
    public CuentaContable getCuentaContableByRow(int index){
        return lista.get(index);
    }
    
    
}
